...
{
  if (v.size() != w.size())
    throw std::logic_error("QPVectors of different size are not confrontable");
...
template <typename T>
class CanonicalBaseQPVector : public QPVector<T>
{
public:
  CanonicalBaseQPVector(unsigned int i, unsigned int n);
...
template <typename T>
CanonicalBaseQPVector<T>::CanonicalBaseQPVector(unsigned int i, unsigned int n)
  : QPVector<T>((T)0, n), e(i)
{ (*this)[e] = (T)1; }

template <typename T>
inline void CanonicalBaseQPVector<T>::reset(unsigned int i)
{ 
  (*this)[e] = (T)0; 
  e = i; 
  (*this)[e] = (T)1;
...
QPMatrix<T> pinv(const QPMatrix<T>& A)
{
  ...
  QPVector<T> W;
  CanonicalBaseQPVector<T> e(0, A.nrows());
  svd(A, U, W, V);
