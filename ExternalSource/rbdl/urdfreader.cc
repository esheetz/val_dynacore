...
bool construct_model (Model* rbdl_model, ModelPtr urdf_model, bool floating_base, bool verbose) {
  ...
  for (j = 0; j < joint_names.size(); j++) {
    ...
    rbdl_joint.name = urdf_joint->name;
    if (urdf_joint->limits) {
      rbdl_joint.limits_exist = true;
      rbdl_joint.limit_lower = urdf_joint->limits->lower;
      rbdl_joint.limit_upper = urdf_joint->limits->upper;
    }
