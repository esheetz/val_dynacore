...
OrientationController::OrientationController() {
    ...
    // set topic names
    ref_topic_ = std::string("nstgro20/reference_orientation");
    ...
    if( msg_frame_name != ref_frame_name_ ) {
        ...
        try {
            ...
            if( !tf_.waitForTransform(msg_frame_name, ref_frame_name_, ros::Time(0), ros::Duration(1.0),
                ...
                ROS_ERROR("%s::updateReferenceOrientation() -- no transform from %s to %s; error: %s",
            ...
            ROS_ERROR("%s::updateReferenceOrientation() -- trouble getting transform from %s to %s; TransformException: %s",
