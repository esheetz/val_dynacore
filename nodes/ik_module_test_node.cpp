...
IKModuleTestNode::IKModuleTestNode(const ros::NodeHandle& nh) {
    ...
    // set up parameters
    nh_.param("task_set", task_set_, std::string("wholebody"));
...
bool IKModuleTestNode::performIKTasks() {
    ROS_INFO("[IK Module Test Node] Performing IK tasks...");

    ROS_INFO("[IK Module Test Node] Solving IK problem...");

    // compute IK solution
    bool ik_result = ik_.solve(q_current_);

    // try hard-coded joints from IK test TODO TODO TODO UNDO THIS
